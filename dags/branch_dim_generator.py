...
# DAG configurations
start_date = datetime(year=2024, month=9, day=11)
...
# Sample UK cities and regions for data generation
...
def generate_random_data(row_num):
    ...
    branch_name = f"Branch_{row_num}"
    branch_address = (
        f"{random.randint(1, 999)} {random.choice(['High St', 'King St', 'Queen St'])}"
    )
    ...
    opening_date_millis = int(random_date.timestamp() * 1000)  # Convert to milliseconds
